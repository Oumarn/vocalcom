...
interface DemoFormSolicitaProps {
  customButtonText?: string;
  showHelpField?: boolean;
}

export default function DemoFormSolicita({ customButtonText, showHelpField = false }: DemoFormSolicitaProps = {}) {
  ...
  useEffect(() => {
    ...
    console.log('[DemoFormSolicita] Campaign value:', campaignValue);
    console.log('[DemoFormSolicita] Resolved region from UTM:', regionKey);

    // Map RegionKey to CalendlyRegion
    const calendlyRegion = mapRegionKeyToCalendly(regionKey);
    console.log('[DemoFormSolicita] Mapped to Calendly region:', calendlyRegion);

    // Get Calendly config for the region
    const config = getCalendlyConfig(calendlyRegion);
    console.log('[DemoFormSolicita] Calendly config:', config);
  ...
  useEffect(() => {
    const email = localStorage.getItem("vocalcom_user_email") || "";
    if(email.trim() !== "") {
  ...
  useEffect(() => {
    ...
    const submitToPardot = async (bookingData?: any) => {
      ...
      if (isSubmitting || submissionCompleted) {
        console.log('[DemoFormSolicita] Skipping duplicate Pardot submission');
        return;
      }
      ...
      try {
        ...
        if (!storedData) {
          console.error('[DemoFormSolicita] No form data found in localStorage');
          isSubmitting = false;
          return;
        }
        ...
        if (bookingData) {
          console.log('[DemoFormSolicita] Including booking details:', bookingData);
        ...
        console.log('[DemoFormSolicita] Submitting to Pardot:', Object.fromEntries(pardotData));
        ...
        console.log('[DemoFormSolicita] Pardot submission completed');
        ...
        console.error('[DemoFormSolicita] Error submitting to Pardot:', error);
      } finally {
        isSubmitting = false;
      }
    };
    ...
    const handleMessage = async (e: MessageEvent) => {
      ...
      if (e.data.event === 'calendly.event_scheduled') {
        console.log('[DemoFormSolicita] Event scheduled:', e.data);
        ...
        if (alreadySubmitted) {
          console.log('[DemoFormSolicita] Already submitted, skipping duplicate');
          return;
        }
        ...
        if (typeof window !== 'undefined') {
          ...
          if (localStorage.getItem('vocalcom_demo_booked')) {
            console.log('[DemoFormSolicita] GTM demo_booked already fired, skipping');
          } else {
            ...
            console.log('[DemoFormSolicita] GTM demo_booked event fired');
          }
        }
    ...
    const handleBeforeUnload = () => {
      ...
      if (bookingCompleted) {
        console.log('[DemoFormSolicita] Booking completed, skipping beforeunload submission');
        return;
      }
      ...
      if (typeof window !== 'undefined' && !localStorage.getItem('vocalcom_demo_booked')) {
        ...
        console.log('[DemoFormSolicita] GTM demo_abandoned event fired');
      }

      // Only submit if not already submitted and data still exists
      const storedData = localStorage.getItem('vocalcom_form_submission');
      if (storedData && !submissionCompleted) {
        console.log('[DemoFormSolicita] User leaving without booking, submitting to Pardot');
  ...
  const handleCountrySelect = (country: { value: string; label: any; phonePrefix: string }) => {
    ...
    console.log('[DemoFormSolicita] Country selected:', country.value);

    // Find the appropriate Calendly config for this country
    const calendlyConfig = getCalendlyConfigByCountry(country.value);
    console.log('[DemoFormSolicita] Calendly config for country:', calendlyConfig);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[DemoFormSolicita] Form data saved to localStorage');
      ...
      console.log('[DemoFormSolicita] GTM form_submitted event fired with user_data');
      ...
      localStorage.setItem('vocalcom_landing_language', locale || 'es');
      pardotFields.landing_language = locale || 'es';
      pardotFields.Landing_Language = locale || 'es';
      ...
      try {
        ...
        console.error('[DemoFormSolicita] Pardot submit failed:', pardotErr);
      }
      ...
      console.error('[DemoFormSolicita] Error processing form:', error);
  ...
  if (showCalendly) {
    ...
    console.log('[DemoFormSolicita] Calendly iframe URL:', iframeUrl);
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              className={`flex-1 px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.phone ? 'border-red-500' : 'border-gray-300'
                }`}
            />
          </div>
          {errors.phone && (
        ...
        <div>
          ...
          <input
            ...
            onChange={handleInputChange}
            className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.company ? 'border-red-500' : 'border-gray-300'
              }`}
          />
          {errors.company && (
        ...
        <div>
          ...
          <input
            ...
            onChange={handleInputChange}
            className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.jobTitle ? 'border-red-500' : 'border-gray-300'
              }`}
          />
          {errors.jobTitle && (
